    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
        <div class="content-wrapper">
            ...
            <div class="col-md-12 grid-margin">
              <div class="row">
                <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                  <h2 class="font-weight-bold">Search result for: <?= $search_query ?></h2>
                </div>
              </div>
            </div>
          </div>
          ...
          $stmt = $db->prepare($sql);
          $stmt->execute([$_SESSION['user_id'], '%' . $search_query . '%']);
          ...
          while($notes = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $search->execute([$notes['note_id'], '%' . $search_query . '%']);
          ...
          $sql = "SELECT * FROM notes
              WHERE is_public = ? AND NOT owner = ? AND title LIKE ?";
          $stmt = $db->prepare($sql);
          $stmt->execute([true, $_SESSION['user_id'], '%' . $search_query . '%']);
          while($notes = $stmt->fetch(PDO::FETCH_ASSOC)) {
            array_push($result, $notes);
          }
          ...
          if(count($result) == 0) {
            echo "No search result found for " . $search_query;
            return;
          } else {
